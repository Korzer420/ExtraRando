...
public class ArcaneEggVictoryCondition : IVictoryCondition
{
    ...
    public string GetLogicTerm() => "ARCANEEGGS";
    ...
    {
        if (RandomizerMod.RandomizerMod.RS.GenerationSettings.PoolSettings.Relics)
        {
            ...
            foreach (AbstractItem item in Ref.Settings.GetItems())
            {
                if (item.IsObtained())
                    continue;
                if (item.name == ItemNames.Arcane_Egg || item.name == ItemNames.Kings_Idol
            ...
            if (leftItems.Count == 0)
                return null;
            string text = "These relics should be hidden in:";
